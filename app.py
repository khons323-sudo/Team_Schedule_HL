...
if not chart_data.empty:
    ...
    fig = px.timeline(
        chart_data, x_start="시작일", x_end="종료일", y="프로젝트명_줄바꿈",
        color="담당자", color_discrete_sequence=custom_colors,
        hover_name="프로젝트명", hover_data=["구분", "Activity", "진행률", "남은기간"],
        title=""
    )
    
    fig.add_trace(go.Scatter(
        x=chart_data["종료일"], y=chart_data["프로젝트명_줄바꿈"],
        text=chart_data["담당자_라벨"], mode="text",
    ...
    fig.update_layout(
        ...
        xaxis_title="", yaxis_title="", barmode='group', bargap=0.2, height=300, 
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        margin=dict(l=10, r=50, t=30, b=10), dragmode="pan", 
    ...
    fig.update_yaxes(
        ...
        tickfont=dict(size=12), showgrid=True, gridcolor='rgba(128, 128, 128, 0.2)', gridwidth=1, layer="below traces"
    )
...
# -----------------------------------------------------------------------------
# 5. 컨트롤 패널 (구버전 호환성 위해 popover 대신 expander 사용)
# -----------------------------------------------------------------------------
# [수정] popover 에러 방지를 위해, 구버전에서도 작동하는 'expander'를 아이콘처럼 작게 활용하거나
# UI를 조금 변경하여 안정성을 확보함. 여기서는 다시 expander를 하단에 배치하여 에러 방지.
...
edited_df = st.data_editor(
    filtered_df, height=dynamic_height, use_container_width=True, num_rows="dynamic",
    column_config={
    ...
    column_order=final_display_cols, hide_index=True, key="data_editor"
)
